...
現在バージョン: **Ver.0.1.4**（2026-02-25 更新）
...
- [x] UI/状態管理/キーボード対応
- [x] 通常電卓（縦）/関数モード（横）レイアウト切替
- [x] `()` ワンタップのスマート括弧入力（開き/閉じを文脈判定）
- [x] ボタン間隔・フォントサイズの最適化（配色は維持）
- [x] ダーク/ライトテーマ切替（設定保持）
- [x] モバイル横向き固定（起動時自動判定、非対応時は縦表示）
- [x] GitHub Actions による Pages デプロイ設定
- [x] UI デザインのリニューアル（高コントラスト配色、ファビコン刷新）
- [x] 計算結果からの連続計算（関数で結果を括弧内に自動挿入、演算子で継続）
- [x] 未閉じ括弧の自動補完表示（薄い閉じ括弧で対応を可視化）
- [ ] 任意機能（履歴/メモリ/PWA など）
